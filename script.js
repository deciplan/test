/* ═══════════════════════════════════════════════════════════
   Deciplan — script.js v5 (multi-étudiants)
   ═══════════════════════════════════════════════════════════ */
...
async function decryptPayload(payload, cin){
  var key=await deriveKey(cin, b64ToBuf(payload.salt), payload.iter||200000);
...
/* ═══════════════════════════════════════════════════════════
   ROUTAGE : lecture de ?e= et chargement du fichier étudiant
   ═══════════════════════════════════════════════════════════ */

var STATE={ eleveId:null, payload:null };

function getEleveId(){
  var params=new URLSearchParams(window.location.search);
  var id=(params.get('e')||'').trim().toLowerCase();
  /* Sécurité : on accepte uniquement [a-z0-9-] */
  if(!id) return null;
  if(!/^[a-z0-9\-]{1,60}$/.test(id)) return null;
  return id;
}

function showLockState(which){
  ['lockHome','lockCin','lockError'].forEach(function(id){
    var el=document.getElementById(id);
    if(el) el.hidden=(id!==which);
  });
}

async function tryLoadPayload(id){
  try{
    var r=await fetch('eleves/'+id+'.json',{cache:'no-store'});
    if(!r.ok) return null;
    return await r.json();
  }catch(e){ return null; }
}
...
async function initRouting(){
  ...
  if(!id){
    /* Pas d'identifiant dans l'URL → page d'accueil */
    showLockState('lockHome');
    return;
  }

  /* Tentative de chargement du fichier étudiant */
  var payload=await tryLoadPayload(id);
  if(!payload){
    /* Fichier introuvable → écran d'erreur */
    showLockState('lockError');
    return;
  }

  /* Fichier trouvé → écran CIN */
  STATE.eleveId=id;
  STATE.payload=payload;
  showLockState('lockCin');
  setTimeout(function(){
...
async function handleUnlock(e){
  ...
  if(!STATE.payload){ setLockMsg('Erreur de chargement. Rechargez la page.','error'); return; }
  ...
  try{
    var data=await decryptPayload(STATE.payload, cin);
...
/* ── REÇU ── */
function recuCell(url){
  if(isEmpty(url)) return '<span class="cell-na">—</span>';
  return '<a class="btn-recu" href="'+esc(url)+'" target="_blank" title="Voir le reçu">📄 Voir</a>';
}
...
function renderTable(cands){
  ...
  tbody.innerHTML=cands.map(function(c,i){
    ...
    return '<tr class="tr-'+(i%2===0?'even':'odd')+'">'
      ...
      +'<td class="tc">'+recuCell(c.recu_url)+'</td>'
...
function renderCards(cands){
  ...
  grid.innerHTML=cands.map(function(c,i){
    ...
    var actions='';
    if(c.lien||!isEmpty(c.recu_url)){
      actions='<div class="card-actions">';
      if(c.lien) actions+='<a class="card-btn-link" href="'+esc(c.lien)+'" target="_blank">🔗 Portail</a>';
      if(!isEmpty(c.recu_url)) actions+='<a class="card-btn-recu" href="'+esc(c.recu_url)+'" target="_blank">📄 Voir reçu</a>';
      actions+='</div>';
    }
...
function bootApp(data){
  /* Compatibilité : data peut être un objet ou un tableau */
  var e=Array.isArray(data)?data[0]:data;
  if(!e) return;
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if(!window.crypto || !window.crypto.subtle){
    showLockState('lockError');
    var t=document.querySelector('#lockError .lock-title');
    if(t) t.textContent='Navigateur non compatible';
    var s=document.querySelector('#lockError .lock-sub');
    if(s) s.textContent='Veuillez utiliser Chrome, Firefox, Safari ou Edge récent.';
    return;
  }
