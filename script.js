...
/* Couleurs solides par thème pour les cellules sticky */
var BG = {
...
function renderTable(rows){
  ...
  tbody.innerHTML=rows.map(function(r){
    ...
    /* Styles inline sur les 2 td sticky — priorité absolue */
    ...
    return '<tr'+rc+'>'
      +'<td style="'+s1+'"><button class="fav-btn'+(fav?' active':'')+'" data-nom="'+sn+'" onclick="window.toggleFav(this)" title="'+(fav?'Retirer':'Ajouter')+'">'+(fav?'⭐':'☆')+'</button></td>'
...
function renderCards(rows){
  ...
  if(!grid) return;
  if(!rows.length){
    grid.innerHTML='<div class="card-empty">'+(showFavsOnly?'⭐ Aucun favori. Cliquez sur ☆ pour en ajouter.':'😔 Aucun résultat.')+'</div>';
    return;
  }
  grid.innerHTML=rows.map(function(r){
    ...
    return '<div class="school-card'+(fav?' card-fav':'')+'">'
      +'<div class="card-header">'
        +'<div class="card-header-left"><span class="badge '+bC[st]+'">'+bL[st]+'</span>'+newBadge+'</div>'
      ...
      +'</div>'
      +'<div class="card-dates"><span>'+periodeShort+'</span><span>'+buildCountdown(dl)+'</span></div>'
...
function render(){
  ...
  allData.forEach(function(r){
    var s=getStatus(r), dl=daysLeft(r);
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  fetch('ecoles_complet.json')
    ...
    .catch(function(e){
      console.error(e);
      var msg='<tr><td colspan="11" class="empty">⚠️ Impossible de charger ecoles_complet.json</td></tr>';
