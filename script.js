...
function showToast(msg){
  ...
  clearTimeout(t._h); t._h=setTimeout(function(){ t.classList.remove('toast-show'); },2200);
}

/* ── Slugs uniques pour le partage (#ecole) ── */
var slugMap={};   /* slug -> nom */
function buildSlugs(){
  slugMap={};
  var used={};
  allData.forEach(function(r){
    var base=(r.abrev||r.nom.split('—')[0]).trim().toLowerCase()
      .normalize('NFD').replace(/[\u0300-\u036f]/g,'')
      .replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
    var slug=base, i=2;
    while(used[slug]){ slug=base+'-'+i; i++; }
    used[slug]=true;
    r._slug=slug; slugMap[slug]=r.nom;
  });
}
window.shareSchool=function(btn){
  var slug=btn.getAttribute('data-slug');
  var url=location.origin+location.pathname+'#'+slug;
  function done(){ showToast('🔗 Lien copié — partagez-le sur WhatsApp !'); }
  if(navigator.clipboard&&navigator.clipboard.writeText){
    navigator.clipboard.writeText(url).then(done,function(){ prompt('Copiez ce lien :',url); });
...
function handleHash(){
  ...
  setTimeout(function(){
    var el=document.querySelector('[data-slug-row="'+slug+'"]');
    if(!el) return;
...
/* ── Mémoire des filtres ── */
var FILTERS_KEY='deciplan_filters';
function saveFilters(){
  try{
    localStorage.setItem(FILTERS_KEY,JSON.stringify({
      search:document.getElementById('search').value,
      cat:document.getElementById('filter-cat').value,
      bac:(document.getElementById('filter-bac')||{}).value||'',
...
function restoreFilters(){
  /* un lien partagé (#ecole) prime : on n'applique pas les filtres mémorisés */
  if(location.hash&&location.hash.length>1) return;
  var s;
  try{ s=JSON.parse(localStorage.getItem(FILTERS_KEY)||'null'); }catch(e){ return; }
  if(!s) return;
  function set(id,v){
  ...
  set('search',s.search); set('filter-cat',s.cat); set('filter-bac',s.bac);
  set('filter-ville',s.ville); set('filter-status',s.status);
}
...
/* ── Panneau Minhaty (bourse) ── */
function initMinhaty(){
  var panel=document.getElementById('minhatyPanel');
  var cd=document.getElementById('minhatyCountdown');
  var sub=document.getElementById('minhatySub');
  if(!panel) return;
  var open=new Date(2026,6,15);   /* 15 juillet 2026 */
  var close=new Date(2026,9,18);  /* 18 octobre 2026 */
  var dOpen=Math.round((open-TODAY)/86400000);
  var dClose=Math.round((close-TODAY)/86400000);
  panel.classList.remove('minhaty-soon','minhaty-closed');
  if(dOpen>0){
    /* pas encore ouvert : compte à rebours vers l'ouverture */
    sub.textContent='Ouverture des dépôts le 15 juillet 2026';
    cd.innerHTML='<span class="mc-num">'+dOpen+'</span><span class="mc-lbl">j. avant ouverture</span>';
  } else if(dClose>=0){
    /* période ouverte : compte à rebours vers la clôture */
    sub.textContent='Dépôt ouvert jusqu\'au 18 octobre 2026 · www.minhaty.ma';
    var cls=dClose<=14?' mc-urgent':'';
    if(dClose<=14) panel.classList.add('minhaty-soon');
    cd.innerHTML='<span class="mc-num'+cls+'">'+(dClose===0?'Dernier jour':dClose)+'</span>'
      +(dClose===0?'':'<span class="mc-lbl">jours restants</span>');
  } else {
    /* clôturé */
    sub.textContent='Les dépôts sont clôturés (échéance : 18 octobre 2026)';
    panel.classList.add('minhaty-closed');
    cd.innerHTML='<span class="mc-num mc-closed">Clôturé</span>';
  }
}
...
/* ── Bandeau urgent : clôtures sous 7 jours ── */
function buildUrgentBanner(){
  var box=document.getElementById('urgentBanner');
  if(!box) return;
  var urgent=allData.filter(function(r){ return getStatus(r)==='bientot'; })
    .sort(function(a,b){ return daysLeft(a)-daysLeft(b); });
  if(!urgent.length){ box.hidden=true; return; }
  var items=urgent.slice(0,4).map(function(r){
    var d=daysLeft(r);
    var ab=r.abrev||r.nom.split('—')[0].trim();
    return '<button class="urgent-item" data-slug="'+r._slug+'">'+ab
      +' <span class="urgent-days">'+(d===0?"auj. !":d+" j")+'</span></button>';
  }).join('');
  var extra=urgent.length>4?'<button class="urgent-extra" id="urgentMore">+'+(urgent.length-4)+' autres — tout voir</button>':'';
  box.innerHTML='<span class="urgent-label">⚠️ Clôturent sous 7 jours :</span>'+items+extra;
  box.hidden=false;
  var more=document.getElementById('urgentMore');
  if(more) more.addEventListener('click',function(){
    document.getElementById('filter-status').value='bientot';
    render(); saveFilters();
    var tw=document.getElementById('table-desktop');
    var cg=document.getElementById('cards-mobile');
    var target=(tw&&tw.offsetParent)?tw:cg;
    if(target) target.scrollIntoView({behavior:'smooth',block:'start'});
    showToast('Filtre « Bientôt fermé » appliqué — '+urgent.length+' écoles');
  });
  box.querySelectorAll('.urgent-item').forEach(function(b){
    b.addEventListener('click',function(){
      var slug=this.getAttribute('data-slug');
      /* montrer l'école : on neutralise le filtre statut si besoin */
      var el=document.querySelector('[data-slug-row="'+slug+'"]');
      if(!el){
      ...
      setTimeout(function(){
        var row=document.querySelector('[data-slug-row="'+slug+'"]');
        if(row){
          row.scrollIntoView({behavior:'smooth',block:'center'});
          row.classList.add('row-flash');
          setTimeout(function(){ row.classList.remove('row-flash'); },2600);
        }
      },100);
    });
  });
...
function renderTable(rows){
  ...
  tbody.innerHTML=rows.map(function(r){
    ...
    return '<tr'+rc+' data-slug-row="'+r._slug+'">'
      +'<td style="'+s1+'"><button class="fav-btn'+(fav?' active':'')+'" data-nom="'+sn+'" onclick="window.toggleFav(this)">'+(fav?'⭐':'☆')+'</button></td>'
      +'<td style="'+s2+'"><div class="school-name">'+r.nom+' '+newBadge
        +'<button class="share-mini" data-slug="'+r._slug+'" onclick="window.shareSchool(this)" title="Copier le lien de cette école">🔗</button></div>'
        +(r.note?'<div class="school-note">'+r.note+'</div>':'')+buildFilieres(r)+'</td>'
...
function renderCards(rows){
  ...
  grid.innerHTML=rows.map(function(r){
    ...
    return '<div class="school-card'+(fav?' card-fav':'')+'" data-slug-row="'+r._slug+'">'
      +'<div class="card-header">'
        +'<span class="badge '+bC[st]+'">'+bL[st]+'</span>'
        +'<span class="card-head-btns">'
        +'<button class="share-mini" data-slug="'+r._slug+'" onclick="window.shareSchool(this)" title="Copier le lien">🔗</button>'
...
/* ══════════════════════════════════════
   INIT — cache-first + revalidation réseau
══════════════════════════════════════ */
var DATA_CACHE_KEY='deciplan_data_v1';
var dataBooted=false;

function bootData(d){
  allData=d;
  buildSlugs();
  buildCats(); buildBacs(); buildVilles();
  if(!dataBooted){ initSortHeaders(); restoreFilters(); }
  updateFavCount(); updateProgress();
  buildUrgentBanner();
  render();
  initCalendar(d);
  if(!dataBooted) handleHash();
  dataBooted=true;
}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  /* 1) Démarrage instantané depuis le cache local s'il existe (protégé) */
  var cached=null;
  try{ cached=JSON.parse(localStorage.getItem(DATA_CACHE_KEY)||'null'); }catch(e){ cached=null; }
  if(cached&&cached.length){
    try{ bootData(cached); }
    catch(e){
      /* cache corrompu ou format périmé : on l'ignore et on le purge */
      console.warn('Cache invalide, purge :',e);
      try{ localStorage.removeItem(DATA_CACHE_KEY); }catch(_){}
      cached=null; dataBooted=false;
    }
  }

  /* 2) Réseau : rafraîchit et met le cache à jour (source de vérité) */
  fetch('ecoles_complet.json?t='+Date.now(),{cache:'no-store'})
    .then(function(r){ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
    .then(function(d){
      try{ localStorage.setItem(DATA_CACHE_KEY,JSON.stringify(d)); }catch(e){}
      /* re-boot si différent du cache, protégé contre toute erreur de rendu */
      if(!cached||JSON.stringify(d)!==JSON.stringify(cached)){
        try{ bootData(d); }
        catch(e){
          console.error('Erreur de rendu :',e);
          var tb=document.getElementById('tbody');
          if(tb) tb.innerHTML='<tr><td colspan="11" class="empty">⚠️ Erreur d\'affichage. Rechargez la page (Ctrl+Shift+R).</td></tr>';
        }
      }
    })
    .catch(function(e){
      console.error(e);
      if(dataBooted) return;   /* le cache a déjà affiché quelque chose */
  ...
  /* Recherche avec debounce (fluidité) + filtres mémorisés */
  var renderAndSave=function(){ render(); saveFilters(); };
  document.getElementById('search').addEventListener('input',debounce(renderAndSave,200));
  document.getElementById('filter-cat').addEventListener('change',renderAndSave);
  var fb=document.getElementById('filter-bac'); if(fb) fb.addEventListener('change',renderAndSave);
  document.getElementById('filter-ville').addEventListener('change',renderAndSave);
  document.getElementById('filter-status').addEventListener('change',renderAndSave);
...
function initCalendar(data){
  ...
  renderCalendar();
  buildCalLegend();
  if(calListenersDone) return;
  calListenersDone=true;
