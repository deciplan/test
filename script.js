...
document.addEventListener('DOMContentLoaded',function(){
  ...
  /* 1) Démarrage instantané depuis le cache local s'il existe */
  var cached=null;
  try{ cached=JSON.parse(localStorage.getItem(DATA_CACHE_KEY)||'null'); }catch(e){}
  if(cached&&cached.length) bootData(cached);

  /* 2) Réseau : rafraîchit et met le cache à jour */
  fetch('ecoles_complet.json?t='+Date.now(),{cache:'no-store'})
    ...
    .then(function(d){
      ...
      /* re-boot seulement si différent du cache (évite un re-render inutile) */
      if(!cached||JSON.stringify(d)!==JSON.stringify(cached)) bootData(d);
    })
    .catch(function(e){
      console.error(e);
